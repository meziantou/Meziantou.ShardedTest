...
internal static class Program
{
    ...
    private static void WriteListTests(IReadOnlyList<DiscoveredTest> tests)
    {
        Console.WriteLine("The following Tests are available:");
        foreach (var test in tests)
        {
            var framework = string.IsNullOrWhiteSpace(test.TargetFramework) ? UnknownFramework : test.TargetFramework;
            Console.WriteLine($"    ({framework}, {test.Name})");
        }
